...
interface CdvNavItem {
  label: string;
  to: string;
  icon: string;
}

interface CdvNavSection {
  label?: string;
  items: CdvNavItem[];
}

// Menu dédié au rôle CDV (Chef Division Ventes) — même traitement compact
// que SidebarCommercial.tsx / SidebarDeveloppeur.tsx / SidebarGraphiste.tsx.
const NAV_SECTIONS: CdvNavSection[] = [
  {
  ...
  },
  {
    ...
    items: [
      { label: 'Prospects', to: '/prospection', icon: 'person_search' },
      { label: 'Clients', to: '/clients', icon: 'apartment' },
      { label: 'Partenariats', to: '/partenariats/apercu', icon: 'handshake' },
    ],
  },
  {
    label: 'Ressources',
    items: [
      { label: 'Validations', to: '/centre-validation', icon: 'verified' },
...
export default function SidebarCdv() {
  return (
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className="font-label-md text-[8px] text-white/40 uppercase tracking-widest">Division Ventes</span>
          </div>
        </div>
