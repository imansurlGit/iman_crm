...
interface VipNavItem {
  label: string;
  to: string;
  icon: string;
}

interface VipNavSection {
  label?: string;
  items: VipNavItem[];
}

// Menu dédié au rôle VIP (Chef Division Visibilité, Infrastructure et
// Production) — même traitement compact que SidebarComptable.tsx / SidebarCdv.tsx.
const NAV_SECTIONS: VipNavSection[] = [
  {
    items: [{ label: 'Tableau de bord', to: '/dashboard', icon: 'space_dashboard' }],
  },
  {
    label: 'Production',
    items: [{ label: 'Suivi production', to: '/vip/production', icon: 'view_kanban' }],
  },
  {
    label: 'Opérations',
    items: [
...
export default function SidebarVip() {
  return (
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className="font-label-md text-[8px] text-white/40 uppercase tracking-widest">Visibilité & Production</span>
          </div>
        </div>
