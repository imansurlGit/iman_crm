...
interface CommercialNavSection {
  label?: string;
  items: CommercialNavItem[];
}

// Menu dédié au rôle COMMERCIAL — priorité donnée au dossier client (l'outil
// de travail quotidien), pas à un empilement de sections génériques.
const NAV_SECTIONS: CommercialNavSection[] = [
  {
  ...
  },
  {
    label: 'Mon activité',
    items: [
      { label: 'Mes dossiers', to: '/dossiers-clients', icon: 'folder_shared' },
      { label: 'Mes projets', to: '/commercial/projets', icon: 'account_tree' },
      { label: 'Mes tâches', to: '/commercial/taches', icon: 'checklist' },
      { label: 'Mon calendrier', to: '/calendrier-collaboratif', icon: 'calendar_month' },
      { label: 'Mes contacts', to: '/mes-contacts', icon: 'contacts' },
    ],
  },
  {
    label: 'Ressources',
    items: [
      { label: 'Mes validations', to: '/commercial/validations', icon: 'verified' },
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className="font-label-md text-[8px] text-white/40 uppercase tracking-widest">Espace Commercial</span>
          </div>
        </div>
