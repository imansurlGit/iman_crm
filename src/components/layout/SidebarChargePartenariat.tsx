...
interface ChargePartenariatNavItem {
  label: string;
  to: string;
  icon: string;
}

interface ChargePartenariatNavSection {
  label?: string;
  items: ChargePartenariatNavItem[];
}

// Menu dédié au rôle CHARGE_PARTENARIAT (Chargé(e) de Partenariat) — même
// traitement compact que SidebarDg.tsx / SidebarCdv.tsx / SidebarCdm.tsx.
const NAV_SECTIONS: ChargePartenariatNavSection[] = [
  {
...
export default function SidebarChargePartenariat() {
  return (
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className="font-label-md text-[8px] text-white/40 uppercase tracking-widest">Partenariats</span>
          </div>
        </div>
