...
interface GraphisteNavItem {
  label: string;
  to: string;
  icon: string;
}

interface GraphisteNavSection {
  label?: string;
  items: GraphisteNavItem[];
}

// Menu dédié au rôle GRAPHISTE — même traitement compact que
// SidebarCommercial.tsx / SidebarDeveloppeur.tsx.
const NAV_SECTIONS: GraphisteNavSection[] = [
  {
  ...
  },
  {
    label: 'Mon activité',
    items: [
      { label: 'Mes tâches', to: '/graphiste/taches', icon: 'checklist' },
      { label: 'Mes projets', to: '/graphiste/projets', icon: 'account_tree' },
      { label: 'Mes validations', to: '/graphiste/validations', icon: 'rate_review' },
...
export default function SidebarGraphiste() {
  return (
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className="font-label-md text-[8px] text-white/40 uppercase tracking-widest">Espace Graphiste</span>
          </div>
        </div>
