...
// "Mes tâches" / "mes projets" du développeur = les Task(task_type=PRESTATION)
// qui lui sont assignées, remontées jusqu'au Project via Prestation. Aucun
// modèle "équipe" ou "charge de travail" dédié n'existe côté backend — RDW
// (RdwWorkspaceContext.tsx) dérive la sienne de la même façon, par division.
//
...
export interface DeveloperTask extends Task {
  projectName: string;
  projectId: number | null;
  submissionStatus: DeveloperTaskStatus;
...
// n'existe côté backend (même mapping que ProjetsPage.tsx/DgPipelinePage.tsx).
...
export const SUBMISSION_STATUS_LABELS: Record<DeveloperTaskStatus, string> = {
  A_FAIRE: 'À faire',
...
export const SUBMISSION_STATUS_CLASSES: Record<DeveloperTaskStatus, string> = {
...
function deriveSubmissionStatus(task: Task, lastDocument: Document | undefined): DeveloperTaskStatus {
...
interface DeveloppeurWorkspaceContextValue {
  tasks: DeveloperTask[];
  projects: DeveloperProject[];
  isLoading: boolean;
  /** Ne clôt réellement la tâche que si sa dernière soumission est validée. */
  markTaskDone: (task: DeveloperTask) => Promise<void>;
}

const DeveloppeurWorkspaceContext = createContext<DeveloppeurWorkspaceContextValue | undefined>(undefined);

export function DeveloppeurWorkspaceProvider({ children }: PropsWithChildren) {
  const { user } = useAuth();
  ...
  const tasks = useMemo<DeveloperTask[]>(
    () =>
      myTasks.map((task) => {
  ...
  const projects = useMemo<DeveloperProject[]>(() => {
    const map = new Map<number, DeveloperProject>();
  ...
  async function markTaskDone(task: DeveloperTask) {
  ...
  return (
    <DeveloppeurWorkspaceContext.Provider value={{ tasks, projects, isLoading, markTaskDone }}>
      {children}
    </DeveloppeurWorkspaceContext.Provider>
  );
}

export function useDeveloppeurWorkspace() {
  const context = useContext(DeveloppeurWorkspaceContext);
  if (!context) {
    throw new Error("useDeveloppeurWorkspace doit être utilisé à l'intérieur de DeveloppeurWorkspaceProvider");
  }
  return context;
}
