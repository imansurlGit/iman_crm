...
export default function DashboardGraphistePage() {
  const navigate = useNavigate();
  const { tasks, projects, documents, isLoading } = useGraphisteWorkspace();

  const pendingTasks = useMemo(() => tasks.filter((t) => !t.done), [tasks]);
  const overdueTasks = useMemo(() => tasks.filter(isOverdue), [tasks]);
  const pendingValidations = documents.filter((d) => d.status === 'A_VALIDER').length;

  const upcomingTasks = useMemo(
    () => [...pendingTasks].sort((a, b) => (a.due_at ?? '').localeCompare(b.due_at ?? '')).slice(0, 5),
    [pendingTasks],
  );
  const recentDocuments = useMemo(
    () => [...documents].sort((a, b) => b.uploaded_at.localeCompare(a.uploaded_at)).slice(0, 4),
    [documents],
  );
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-gutter">
      <section>
        ...
        <p className="text-secondary mt-1 text-sm">Mes tâches, mes projets et le suivi de mes fichiers envoyés en validation.</p>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter">
        ...
        <div className={`${CARD_CLASSES} p-4 flex items-center gap-3`}>
          <span className="material-symbols-outlined text-amber-600 text-2xl shrink-0">rate_review</span>
          <div className="min-w-0">
            <span className="text-secondary text-xs font-medium block truncate">En attente de validation</span>
            <span className="font-headline-md text-headline-md text-on-surface leading-tight">{pendingValidations}</span>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-12 gap-gutter">
        ...
        <div className={`${CARD_CLASSES} col-span-12 lg:col-span-7 p-6`}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-xs font-bold text-primary hover:underline" onClick={() => navigate('/graphiste/taches')} type="button">
        ...
        {/* Résultats de mes validations */}
        <div className={`${CARD_CLASSES} col-span-12 lg:col-span-5 p-6`}>
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-headline-md text-lg font-bold text-primary">Résultats de mes validations</h4>
            <button className="text-xs font-bold text-primary hover:underline" onClick={() => navigate('/graphiste/validations')} type="button">
              Voir tout
            </button>
          </div>
          <div className="space-y-2.5">
            {recentDocuments.map((document) => (
              <div className="px-3.5 py-3 bg-surface-container-low rounded-lg" key={document.id}>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-on-surface truncate">{document.label}</p>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase shrink-0 ${RESULT_CLASSES[document.status]}`}>
            ...
            {recentDocuments.length === 0 && <p className="text-sm text-secondary">Aucun envoi pour l'instant.</p>}
          </div>
        </div>
      </section>
      ...
      <section className={`${CARD_CLASSES} p-6`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="text-xs font-bold text-primary hover:underline" onClick={() => navigate('/graphiste/projets')} type="button">
        ...
        <div className="space-y-2">
          {projects.map((project) => (
            <div className="flex items-center gap-3 px-3.5 py-3 bg-surface-container-low rounded-lg" key={project.id}>
              ...
              <span className="text-[11px] text-secondary shrink-0 w-24 text-right">
                {project.deadline ? formatDate(project.deadline) : 'Sans échéance'}
              </span>
            </div>
          ))}
          {projects.length === 0 && (
            <p className="text-center text-secondary text-sm py-4">Aucun projet assigné pour l'instant.</p>
          )}
        </div>
      </section>
    </div>
