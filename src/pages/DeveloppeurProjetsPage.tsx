...
import { useDeveloppeurWorkspace } from '../context/DeveloppeurWorkspaceContext';

function formatDate(value: string): string {
  return new Date(value).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' });
}

export default function DeveloppeurProjetsPage() {
  const navigate = useNavigate();
  const { projects, isLoading } = useDeveloppeurWorkspace();

  return (
    <div className="flex flex-col gap-gutter">
      <section>
        <h2 className="font-headline-md text-headline-md text-on-surface tracking-tight">Mes projets</h2>
        <p className="text-secondary mt-1 text-sm">Les projets de développement sur lesquels vous intervenez.</p>
      </section>
                ...
                <tr className="hover:bg-surface-container-lowest transition-colors" key={project.id}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      onClick={() => navigate(`/developpeur/projets/${project.id}`)}
                      type="button"
                    >
                      Détail
